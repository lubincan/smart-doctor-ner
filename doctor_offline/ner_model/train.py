...
def train(data_loader, data_size, batch_size, embedding_dim, hidden_dim,
    ...
    # 初始化存放训练中损失, 准确率, 召回率, F1等数值指标(精确率和召回率的调和平均数，旨在平衡这两个指标)
    train_loss_list = []
    train_acc_list = []
    train_recall_list = []
    train_f1_list = []
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in tqdm(data_loader["train"]):
            # 将数据以Variable进行封装,Variable 是一个包装类，用于封装张量，并支持自动求导功能
        ...
        '''model.train(),model.eval()不设置模型模式会导致模型在评估时继续使用训练模式的行为，这可能会影响模型性能的评估。因此，建议在训练和评估阶段显式调用 model.train() 和 model.eval() 以确保模型的行为符合预期。这样可以获得更稳定和可靠的评估结果'''
        # 开启当前epochs的验证部分
        with torch.no_grad():
